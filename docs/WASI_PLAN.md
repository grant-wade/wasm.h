...
- [ ] Broaden the core-instance path beyond the current supported subset. Sequential instantiation, top-level export aliases, `from exports`, and direct core singleton args already work on the narrow path, including unnamed singleton exports or local lower targets when the destination import name is unique; the remaining core-instance forms used by real composed components still need to link instead of trapping out as unsupported.
