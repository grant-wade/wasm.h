...
- [DONE] **Core type AST** — retain top-level and nested `core:type` entries as structured nodes: module types, rec groups, subtypes, and their inner composite kinds (`func`, `struct`, `array`, `cont`) plus referenced indices where present.
- [DONE] **Core module type declarations** — inside core module types, retain import, type, alias, and export declarations with their extern-type payloads so M5 can do type-directed linking instead of byte-level reparsing.
...
**Status:** In progress. The runtime still does not execute the full general component linking program, but the current `wasi_instantiate` path now has an active supported `value`-linking slice in addition to the earlier `func`/`instance`/`component`/`type` work. The current component-instance linking slice executes local `component instance` instantiate records, recursively instantiates nested child components, binds `module`/`func`/`instance`/`component`/`type`/`value` arg maps into child imports, resolves component function and value aliases through live child instances rather than only through static `instance { export ... }` tables, resolves component-exported `type` args through live child instances on the supported path, resolves non-core outer aliases for `func`, `instance`, `component`, `type`, and `value` on the supported path, resolves outer core-module aliases on the supported path for nested core-instance wiring, resolves component-defined resource destructors through the same live alias-routing path including nested outer function aliases, resolves top-level component imports against engine-bound bindings by fully qualified name/version via `wasi_bind_import_instance`, `wasi_bind_import_module`, `wasi_bind_import_func`, `wasi_bind_import_component`, and `wasi_bind_import_value`, structurally checks linked component values against dynamically resolved expected types, transfers `own<T>` resource handles into destination instances with automatic destination resource binding, and executes zero-arg/zero-result component start functions on the supported path. The current core-instance linking slice remains active as well: sequential core-instance instantiation, top-level core-instance export aliases for `canon lift` including aliases that hop through top-level core-instance `from exports` namespaces, top-level core-instance `from exports` namespaces backed by direct forwards or synchronous `canon lower` thunks, whole-namespace `from exports` links whose unnamed singleton exports can derive their field names from the destination core module when the match is unique, including uniquely matched empty import fields, named singleton re-exports that can stay linked through the generic core export resolver even when the resolved function target bottoms out at a synchronous `canon lower`, direct core singleton args that can bind the same forward-or-lower targets as singleton imports for later embedded core modules, including unnamed local targets when the destination core module exposes a unique matching import field even if that field name is empty, and singleton import-name resolution that can recurse through outer or instance-exported module aliases in runtime state instead of dropping to `WASI_ERR_NOT_IMPLEMENTED`. Broader core-type aliases, more exotic core-instance forms, and `borrow<T>`-containing linked values still remain later work.

### Current Supported Slice

The active M5 implementation is no longer just `func`/`instance`/`component`/`type` linking. The current supported path now includes:

- Nested `component instance` execution with recursive child instantiation.
- Child arg-map binding for `module`, `func`, `instance`, `component`, `type`, and `value` imports.
- Value linking for top-level and nested component imports, including structural type checks, deep-copy behavior for heap-backed values, and `own<T>` resource transfer with destination-instance rebinding.
- Resolution of component function, value, and type aliases through live child instances on the supported path.
- Non-core outer aliases for `func`, `instance`, `component`, `type`, and `value` on the supported path.
- Component-defined resource destructor dispatch through local aliases, live child-instance aliases, and nested outer function aliases on the supported path.
- Outer core-module aliases plus the current narrow core-instance path: sequential core-instance instantiation, top-level core-instance export aliases for `canon lift`, top-level `from exports` namespaces backed by direct forwards or synchronous `canon lower` thunks, whole-namespace `from exports` links with unnamed singleton exports when the destination import name is uniquely implied by the linked core module, including empty import fields, named singleton re-exports that can stay linked through alias chains even when the resolved function target is a synchronous `canon lower`, and direct core singleton args bound to the same forward-or-lower targets, including unnamed local sources when the destination import name is uniquely implied by the linked core module.
- Top-level import resolution through `wasi_bind_import_instance`, `wasi_bind_import_module`, `wasi_bind_import_func`, `wasi_bind_import_component`, and `wasi_bind_import_value` using fully qualified interface name/version matching.
- Zero-arg/zero-result component start execution on the supported path.

### Current Validation Snapshot

As of 2026-04-13, the current M5-adjacent test surface looks like this:
...
- The current `wasi_test` matrix covers top-level host value imports, imported-type-backed nested value args, own-resource-containing linked values, explicit `borrow<T>` rejection cases on the current path, component resource destructor dispatch through instance-export aliases and nested outer function aliases, canon-lift resolution through top-level core-instance `from exports` namespaces backed by both direct forwards and synchronous `canon lower` thunks, whole-namespace `from exports` links with unnamed function exports or empty destination import fields, named singleton re-exports that stay linked through a `from exports` alias chain when the resolved target is a synchronous `canon lower`, direct core `func` args sourced from unnamed local `canon lower` definitions including empty destination import fields, and direct singleton import-name resolution through outer or instance-exported module aliases in runtime fixtures.
...
- **Finish the remaining linking-program coverage in `wasi_instantiate`.** M5 is still open until the remaining broader core-alias/core-instance forms and other fallback `WASI_ERR_NOT_IMPLEMENTED` exits on the general linking path are either implemented or explicitly removed from scope.
- **Broaden the core-instance wiring beyond the current narrow supported slice.** Today the runtime supports sequential core-instance instantiation, top-level export aliases, top-level `from exports`, named singleton re-exports that resolve through the current alias-chain path even when they bottom out at a synchronous `canon lower`, and direct core singleton args on the supported path, including unnamed singleton exports, empty destination import fields, or local lower targets when the destination import name is unique. The remaining core-instance forms used by real composed components still need to link instead of failing as unsupported.
- **Resolve the retained `borrow<T>` question.** `wasi.h` can already lower synchronous borrows during `wasi_call`, but linked component values still reject `borrow<T>` because there is no safe long-lived retained-borrow model for stored linker values. Either that model needs to land or the M5 scope needs to be narrowed explicitly.
- **Prove the broader supported path with end-to-end validation.** Parser/smoke coverage is in good shape, but M5 still needs stronger composed-component validation for alias/export routing, multi-module/shared-core wiring, and supported start execution.
- **Keep the error surface consistent across the remaining paths.** Unresolved imports and structural type mismatches should keep producing named, actionable diagnostics instead of dropping into generic `not implemented` or opaque traps.

#### M5 Closeout Checklist

- [x] Keep the current supported slice green in the native/unit matrix. `wl_test`, `wasm_test`, `wasi_test`, the current `wasi-component` smoke/introspection coverage, the spectest pass, and the current emcc fixtures should all stay green while the remaining linker work lands.
- [ ] Finish the remaining `wasi_instantiate` linking-program coverage needed by real composed components. The milestone cannot close while the general path still falls back to `WASI_ERR_NOT_IMPLEMENTED` for required alias or core-instance forms.
- [ ] Broaden the core-instance path beyond the current supported subset. Sequential instantiation, top-level export aliases, `from exports`, named singleton re-exports that resolve through the current alias-chain path even when they bottom out at a synchronous `canon lower`, and direct core singleton args already work on the narrow path, including unnamed singleton exports, empty destination import fields, or local lower targets when the destination import name is unique; the remaining core-instance forms used by real composed components still need to link instead of trapping out as unsupported.
