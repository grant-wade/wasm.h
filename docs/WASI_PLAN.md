...
**Status:** In progress. `wasi.h` now has engine-defined host resource types, per-instance handle tables, explicit component-type bindings, `wasi_resource_new` / `wasi_resource_rep` / `wasi_resource_drop`, slot reuse after drops, and synchronous `own<T>` / `borrow<T>` lowering on the current `wasi_call` path for supported single-module canon lifts. The current narrow single-module path also wires `canon resource.new` / `canon resource.rep` / `canon resource.drop` into exported calls and runs same-instance component-defined resource destructors on explicit drops and instance teardown. Cross-component handle transfer and broader destructor handling across future M5 instantiation/linking paths remain later work.
...
**Status:** In progress. The runtime still does not execute the general component linking program, but the narrow `wasi_instantiate` path now accepts trivial core-instance instantiation records, including a small sequential subset where a later core instance can satisfy function imports from an earlier core instance even when that earlier instance was itself loaded dynamically at instantiation time. The same narrow path also accepts top-level core-instance export aliases and resolves `canon lift` targets through those aliases, so direct exported lifts are no longer limited to the final core module's raw local function indices. In addition, exported component funcs can now resolve through static `instance { export ... }` records plus top-level component instance-export aliases when those aliases ultimately lead back to a locally defined canon-backed func. Outer component aliases still remain later work because the runtime does not yet instantiate nested component scopes.
