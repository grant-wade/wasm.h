...
static int spec_message_matches(const char* expected, const char* actual) {
    ...
    if (strcmp(expected_lower, "incompatible import type") == 0 &&
        ...
        return 1;
    if (strcmp(expected_lower, "sub type") == 0 &&
        (strstr(actual_lower, "cannot extend final type") != NULL ||
         strstr(actual_lower, "kind does not match supertype") != NULL ||
         strstr(actual_lower, "is not a valid subtype of") != NULL))
        return 1;
