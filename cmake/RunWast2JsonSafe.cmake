if(NOT DEFINED WL_WAST2JSON OR NOT DEFINED WL_INPUT OR NOT DEFINED WL_OUTPUT_JSON OR NOT DEFINED WL_STATUS)
    message(FATAL_ERROR "RunWast2JsonSafe.cmake requires WL_WAST2JSON, WL_INPUT, WL_OUTPUT_JSON, and WL_STATUS")
endif()
...
execute_process(
    COMMAND "${WL_WAST2JSON}" --enable-all "${WL_INPUT}" -o "${WL_OUTPUT_JSON}"
...
if("${WL_RESULT}" STREQUAL "0")
    ...
    if(NOT EXISTS "${WL_OUTPUT_JSON}")
        message(FATAL_ERROR "wast2json succeeded but did not create ${WL_OUTPUT_JSON}")
    endif()
else()
    file(REMOVE "${WL_OUTPUT_JSON}")
    file(WRITE "${WL_OUTPUT_JSON}" "{\n  \"source_filename\": \"${WL_INPUT}\",\n  \"commands\": []\n}\n")
    file(WRITE "${WL_STATUS}" "skip\nwast2json failed for ${WL_INPUT}\n${WL_COMBINED_LOG}")
endif()