...
`wasi.h` still does not implement the full general component linking program. Today it provides parser/introspection, a low-level canonical ABI layer, completed Milestone 4 resource lifecycle support, and two active M5 slices. The current component-instance linking slice can instantiate nested component instances, bind their `func`/`instance`/`component`/`type` arg maps, resolve top-level component imports against engine-bound names and fully qualified interface versions, route exported component functions through live child-component instances rather than only static `instance { export ... }` records, resolve component-exported `type` args through live child-component instances as well as outer aliases on the supported path, execute zero-arg/zero-result component start functions on the current path, resolve non-core outer aliases for `func`, `instance`, and `component`, and resolve non-core outer `type` aliases where they matter on the current path for export type introspection and type-directed lower-bridge metadata. The current core-instance linking slice still materializes top-level core-instance `from exports` namespaces backed by direct forwards or synchronous `canon lower` thunks and still performs the sequential core-instance instantiation needed by later embedded core modules. Broader outer-alias sorts and broader core-instance sort coverage still remain outside the supported path.
